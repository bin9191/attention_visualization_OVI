...
class FusionModel(nn.Module):
    ...
    def single_fusion_cross_attention_forward(self,
        ...
        # Text-to-Video cross attention with optional visualization
        if store_attention and hasattr(cross_attn_block, 'attention_store') and cross_attn_block.attention_store is not None:
            from .attention import attention_with_weights
            x, _, full_attn_weights = attention_with_weights(q, k, v, k_lens=context_lens, average_for_q=False, total_video_latent_frames=31)
            if hasattr(cross_attn_block.attention_store, 'store'):
                # Store full attention weights [B, H, Lq, Lk] for spatial visualization
                cross_attn_block.attention_store.store(full_attn_weights, q.shape, k.shape, src_grid_sizes)
        else:
    ...
    def forward(
        ...
        # Add store_attention to kwargs
        kwargs['store_attention'] = store_attention
