...
# 전역 설정: config에서 설정할 수 있도록
USE_FLASH_ATTENTION = True  # 기본값은 True (기존 동작 유지)

def set_flash_attention_enabled(enabled: bool):
    """
    Flash attention 사용 여부를 설정합니다.

    Args:
        enabled (bool): True면 flash attention 사용, False면 표준 attention 사용
    """
...
def flash_attention(
    ...
    """
    # Config 설정에 따라 표준 attention으로 fallback
...
def attention_with_weights(
    ...
    # Return full attention weights for spatial visualization
    return out.to(out_dtype), avg_attn_weights.to(out_dtype), attn_weights.to(out_dtype)
...
def attention(
    ...
    # 표준 PyTorch attention 사용 (flash_attention에서 호출될 때 사용)
